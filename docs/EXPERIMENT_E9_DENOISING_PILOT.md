...
| 模式 | 输入 | 监督信号 | 对应 |
|---|---|---|---|
| `next_token` | 干净序列 | 答案位置 CE（e3 协议） | 现状（transformer 标准） |
| `denoise` | **加噪序列**（位翻转 p=0.15） | 答案位置 CE——**去噪在状态级**：加噪输入流经循环，模型须滤噪后给出干净答案 | 预测编码：corrupted→clean（状态级） |
| `progressive` | 加噪序列 | **K=3 步逐步修正**：每步输入=上一步 argmax 输出，每步答案 CE | 类扩散：逐步去噪 |
...
- **协议勘误（2026-09-19）**：v1（6K 步）与 v2（20K 步，全位置 CE）均判无效——全位置
  CE 把答案梯度稀释 1/32（31 个随机 bit 位置 + 1 个答案），模型不进 grok；v3 改为
  e3 验证过的答案-only 监督，去噪发生在状态级。**预算教训 = ADJ-001 的 budget_wall
  又一次命中**：先验证基线（next_token）可学，再谈监督信号对比。
